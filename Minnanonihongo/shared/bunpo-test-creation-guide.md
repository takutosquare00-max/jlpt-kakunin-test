...
**推奨（2026-07-23〜の標準）：`unit26-30/unit27/bunpo/bunpo01-kanou-doushi.html` をテンプレ元に使う。**
このファイルはMacBook半画面（Split View、幅約750px前後）で気持ちよく収まるように調整済みの最新デザイン。旧テンプレ（`unit21/bunpo/bunpo01-to-omoimasu.html`）は参考にしない。

### 5-1. 共通機能（既に組み込み済み）
- 上部にタイマー（一時停止 ⏸/▶ ボタン付き）
- プログレスバー・残り問題数表示
- 採点ボタンで一括採点 → 解説の自動表示（採点ボタンは非sticky。全問スクロールし終えた最下部でのみ現れる）
- 選択肢の自動シャッフル（読み込み時）
- 漢字ルビ・モバイル対応
- ヘッダーのタイトルは中央揃え（タイマーは右側固定のまま動かさない）

### 5-1-1. 半画面最適化デザイン（2026-07-23確立、重要）
ユーザーからの反復フィードバックで確立した仕様。新規ファイル作成・改修時は必ずこの仕様に合わせる。

- **コンテナ幅**：`max-width:920px`（`width:100%`）。固定pxで狭めず画面幅に追従させ、半画面(約750px)で左右の余白がほぼ出ないようにする
- **基準フォント**：`html{font-size:21px}`（本文各所は既存のem指定のまま、基準だけ引き上げて全体を拡大）
- **問題番号（丸バッジ `.q-num`）**：`width/height:1.6em`・`vertical-align:middle`・`position:relative;top:-.06em`で微調整。ふりがなの有無で位置がズレないよう、**ふりがな用の余白は予約しない**（ふりがなは行の上に自然にはみ出させてよい、という考え方で統一）
- **選択肢番号（`.opt-key`）**：`.option label`は`align-items:baseline`。`.opt-key`に`position:relative;top:.16em`、さらに**ふりがなを含まない選択肢の`.opt-body`**には`:not(:has(ruby))`で`position:relative;top:.16em`を追加し、ふりがな有無に関わらず選択肢テキストの位置を揃える
- **選択肢の列数自動判定**：JSで各選択肢の文字数（ふりがな＝`rt`/`rp`を除いた実表示文字数）を測定し、**15文字以内なら2列2行、それを超えたら1列**に自動切り替え（`layoutOptions()`関数、`shuffleAllOptions()`より前に実行）。個別に`style="grid-template-columns:1fr"`を手書きする必要はない
- **選択肢番号・本文のフォント**：両方とも`font-family:inherit`で明示的に統一（番号だけ違うフォントに見える問題の対策）
- **採点ボタン**：`position:sticky`は使わない（誤って早期に見えてしまうため）。通常のブロック要素として最後に配置し、全問スクロールし終えた時だけ見えるようにする

この仕様の正確なCSS/JSは`unit27/bunpo01-kanou-doushi.html`を直接参照すること（差分としてまとめるより実物を見る方が確実）。
