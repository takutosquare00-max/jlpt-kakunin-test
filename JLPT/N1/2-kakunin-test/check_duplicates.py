...
def extract_questions(html_path):
    ...
    for block in question_blocks[1:]:
        ...
        if q_num == 10:
            ...
            if passage_match:
                passage = re.sub(r'<[^>]+>', '', passage_match.group(1)).strip()[:150]
...
def main():
    ...
    html_files = sorted([f for f in base_dir.glob('n1-10min-test-v*.html')])
        ...
        try:
            ...
            for q in questions:
                ...
                if sig in all_questions:
                    ...
                    duplicates.append({
                        'dup': (str(html_file.name), q['num'], q['text'][:60], q['correct'][:25]),
    ...
    print("=" * 60)
    print("N1 確認テスト 重複チェック結果")
    print("=" * 60)
    print(f"チェック対象: {len(html_files)} ファイル")
    print(f"総問題数: {sum(len(extract_questions(f)) for f in html_files)}")
    print(f"ユニーク問題数: {len(all_questions)}")
    print()
    
    if duplicates:
        print("⚠️ 完全に同じ問題の重複が見つかりました:")
        for d in duplicates:
